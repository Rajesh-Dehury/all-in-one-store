...
$ctg_edit = $_GET['ctg_edit'];
  $single_get_qry = mysqli_query($conn, "SELECT * FROM select_product WHERE order_number='$ctg_edit' && status='1'");
...
?>
                    ...
                    <div style="text-align: right;">
                      <button class="btn-success" onclick="sub_product_provide('<?php echo $single_get_qry_a['order_number']?>')">Product Provide Submit</button>
                    </div>
<script type="text/javascript">
  function sub_product_provide(order_number)
  {
    // alert(order_number);
    // return false;
    if (confirm("Do you went provide these products ?")) {
      window.location.href="../database/product_provide.php?sub_product_provide_id="+order_number+'';
    }
  }
</script>
                  </div>
