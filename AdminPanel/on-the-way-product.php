...
<?php
  ...
  // while ($banner_qry_a = mysqli_fetch_array($banner_qry)) {
    ...
    $banner_qry = mysqli_query($conn, "SELECT distinct order_number FROM select_product WHERE status='2'");
  while ($banner_qry_a = mysqli_fetch_array($banner_qry)) {
...
?>
                        <tr>
                          ...
                          <td><a href="view-on-the-way-product?ctg_edit=<?php echo $banner_qry_a['order_number']?>"> <span style="padding: 2px 8px 2px 8px; background: green; color: #fff;">View with Provide</span><!-- <i class="fa fa-tasks" style="color: green; cursor: pointer; font-size: 17px;"></i> --></a></td>
                        </tr>
<?php
  }
?>
